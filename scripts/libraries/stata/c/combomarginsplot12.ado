*! combomarginsplot12.ado - Nicholas Winter
*! version 0.1.7 11jan2021

* Changes
* 0.1.7: maintained as combomarginsplot12 for compatibility; incorporated cmp_marginsplot.
* 0.1.3: downgraded to require Stata 12 r/t 13
* 0.1.5: added -restore- to allow addplot() option to work
* 0.1.6: added mlabfmt() option 

program combomarginsplot12
	version 12
	...
	, [ 
		...
		alpha						// alpha-release test feature not to add file var to atlist
		* 
	]
	;
	#delimit cr
...
//RUN -marginsplot-
	restore
	local cmd cmp_marginsplot12 using "`savefile'" , filevarname(`filevarname') `mpopts' `mpsaving' 
	`debug' di `"[`cmd']"'
